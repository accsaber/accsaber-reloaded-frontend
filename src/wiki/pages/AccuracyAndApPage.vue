...
<template>
  <WikiProse>
    <p>
      ...
      Points. There is no randomness in it and nothing hidden, just two ingredients and one
      curve, and once you have seen the curve, AP stops feeling arbitrary forever.
    </p>
    ...
    <p>
      ...
      allows, and it is kept at full precision the whole way through. A 94.63% is a 94.63%, no
      rounding, no cutoffs.
    </p>
    <p>
      The second is the map's complexity, which is AccSaber's own measure of how demanding a map
      is to play accurately, on a scale from 0 to 14. Star ratings and pass difficulty play no
      part in it, and a map that is trivial to survive can still be brutal to acc, which is the
      only half of the question complexity has ever cared about. It is assigned when a map gets ranked, and the harder a map is to acc, the
    ...
    <p>
      ...
      curve's defining feature is how steep it gets near the top. Drag the point around and the
      ...
      and the curve pays accordingly.
    </p>
    <WikiCurveExplorer
      :fn="rawAp"
      :x-min="0.9"
    ...
    <p>
      ...
      and pushing that easy map far enough into the steep end starts it paying like a hard one.
    ...
    <p>
      The 98% on complexity 10 beats the 99% on complexity 3. Raw accuracy alone does not carry
      a score, and yet the last row squeezes that same easy map up to 99.5% and beats everything
      else on the table. Two roads lead to big AP. You can play harder maps you can still acc
      well, or you can lock in and truly master the maps you already own, and the strongest
      players work both at once.
    </p>

    <WikiHeading id="ap-vs-weighted">Two kinds of AP</WikiHeading>
    <p>
      One last thing, and it causes more confusion than everything above combined. The AP on a
      ...
      while improving your number one sends it flying. How that weighting works is the
