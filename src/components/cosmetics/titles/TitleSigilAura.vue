...
function ring(ctx: CanvasRenderingContext2D, cx: number, cy: number, r: number, width: number, alpha: number): void {
...
function ticks(ctx: CanvasRenderingContext2D, cx: number, cy: number, r: number, count: number, rot: number, len: number, alpha: number): void {
  ctx.strokeStyle = withAlpha(color.value, alpha)
  ctx.lineWidth = 1
  for (let i = 0; i < count; i++) {
    const a = rot + (i / count) * Math.PI * 2
    const ca = Math.cos(a)
    const sa = Math.sin(a) * 0.42
    ctx.beginPath()
...
function triangle(ctx: CanvasRenderingContext2D, cx: number, cy: number, r: number, rot: number, alpha: number): void {
  ctx.strokeStyle = withAlpha(color.value, alpha)
  ctx.lineWidth = 1
  ctx.beginPath()
  for (let i = 0; i < 3; i++) {
...
useElementCanvas(canvasRef, {
  ...
  draw(ctx, w, h, now, reduced) {
    ...
    const R = rect.w * 0.62 + rect.fs * 0.4
    ...
    ring(ctx, cx, cy, R, 1.2, 0.5 + 0.3 * pulse)
    ring(ctx, cx, cy, R * 0.82, 0.8, 0.35)
    ticks(ctx, cx, cy, R * 0.82, 24, rot, R * 0.06, 0.5)
    ticks(ctx, cx, cy, R, 8, -rot * 0.5, R * 0.1, 0.6)
    triangle(ctx, cx, cy, R * 0.8, rot * 0.35, 0.45)
    triangle(ctx, cx, cy, R * 0.8, rot * 0.35 + Math.PI, 0.45)
    ring(ctx, cx, cy, R * 0.45, 0.8, 0.3 + 0.3 * pulse)
