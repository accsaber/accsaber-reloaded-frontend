...
useElementCanvas(canvasRef, {
  ...
  draw(ctx, w, h, now, reduced) {
    ...
    for (let cb = 0; cb < 8; cb++) {
      const bx = (cb / 7) * w
      ctx.beginPath()
      ctx.ellipse(bx, 2 * s, (28 + sinHash01(cb) * 14) * s, (12 + sinHash01(cb + 9) * 6) * s, 0, 0, Math.PI * 2)
      ctx.fill()
    }

    for (let vn = 0; vn < 7; vn++) {
      const vx = (0.1 + vn * 0.13) * w + sinHash01(vn + 20) * 8 * s
      const vlen = (16 + sinHash01(vn + 33) * 38) * s
    ...
    for (let ff = 0; ff < 12; ff++) {
      const px = sinHash01(ff + 50) * w + 12 * s * Math.sin(t * 0.2 + ff)
      const py = 26 * s + sinHash01(ff + 70) * (h - 45 * s) + 7 * s * Math.sin(t * 0.31 + ff * 2)
