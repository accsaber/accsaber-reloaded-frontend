...
useElementCanvas(canvasRef, {
  ...
  draw(ctx, w, h, now, reduced) {
    ...
    for (let b = 0; b < 3; b++) {
      const bx = ((sinHash01(b + 3) + t * 0.008 * (1 + b * 0.3)) % 1.1) * w
    ...
    if (!reduced) {
      ...
      for (let bk = 0; bk < 3; bk++) {
        ...
        const tier = Math.floor(sinHash01(cycId + 11) * 4)
        const slotX = w / 2 + (sinHash01(cycId + 23) - 0.5) * WIDTHS[tier] * w * 0.6
        const slotY = h - (tier + 1) * tierH + 6 * s
        const side = sinHash01(cycId + 37) > 0.5 ? w * 0.06 : w * 0.94
        if (u < 0.72) {
          const pr = u / 0.72
