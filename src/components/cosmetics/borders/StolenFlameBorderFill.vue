...
useElementCanvas(canvasRef, {
  init() {
    braziers = Array.from({ length: 12 }, (_, i) => ({
      x: 0.06 + sinHash01(i * 3 + 1) * 0.88,
      y: 0.06 + sinHash01(i * 7 + 2) * 0.88,
      litAt: -1e9,
    }))
    trail = []
    embers = []
  ...
  draw(ctx, w, h, now, reduced) {
    ...
    if (bx < 0) {
      ...
      const ang = sinHash01(7) * Math.PI * 2
      dirX = Math.cos(ang)
      dirY = Math.sin(ang)
    ...
    if (bounced) {
      ...
      for (let k = 0; k < 6 && embers.length < 34; k++) {
        embers.push({
          x: bx,
          y: by,
          vx: (sinHash01(now + k) - 0.5) * 2.4,
          vy: (sinHash01(now * 3 + k) - 0.5) * 2.4 - 0.6,
          life: 1,
        })
      }
    ...
    if (!reduced && now - lastTrail > 45) {
      lastTrail = now
      trail.push({ x: bx, y: by, bornAt: now, seed: sinHash01(now) * 40 })
    ...
    for (let sp = 0; sp < SPARK_COUNT; sp++) {
      const sx = toX((sinHash01(sp + 90) * 100 + t * (2 + sinHash01(sp + 30) * 3)) % 100)
      const sy = toY((sinHash01(sp + 55) * 100 - t * (1.4 + sinHash01(sp + 70) * 2) % 100 + 100) % 100)
      const flick = 0.25 + 0.45 * Math.max(0, Math.sin(t * (1.6 + sinHash01(sp) * 1.8) + sp * 2.3))
    ...
    if (!reduced && now - lastEmber > 70 && embers.length < 34) {
      ...
      embers.push({
        x: bx,
        y: by - cell * 0.3,
        vx: (sinHash01(now) - 0.5) * 0.9 - dirX * 0.5,
        vy: -0.5 - sinHash01(now * 3) * 0.8,
        life: 1,
      })
    }
    ...
    if (!reduced && now - lastSmoke > 420 && smokes.length < 6 && trail.length > 4) {
      ...
      smokes.push({ x: src.x, y: src.y, vx: (sinHash01(now * 11) - 0.5) * 0.3, bornAt: now, seed: sinHash01(now * 5) * 20 })
    }
